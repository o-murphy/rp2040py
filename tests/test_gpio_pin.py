...
def test_undriven_pin_with_pulldown_resolves_low(rp2040_factory):
    gpio = _gpio(rp2040_factory)
    gpio.pad_value = _INPUT_ENABLE | _PULLDOWN
    assert gpio.input_value is False


def test_actively_driven_pin_ignores_pullup(rp2040_factory):
...
def test_actively_driven_pin_survives_input_enable_toggling(rp2040_factory):
...
def test_bus_keeper_mode_falls_back_to_raw_value_when_undriven(rp2040_factory):
