...
18 worked `--board-spec` targets for real third-party hardware live in [boards/](boards/) - every
...
- **A real extension point for third-party hardware** (`ExternalDevice`/`boards.BoardSpec`) -
  ...
  scripts. rp2040py ships 18 worked `--board-spec` examples for real vendor boards (WeAct Studio,
  four Waveshare boards, VCC-GND Studio, three Adafruit boards, McHobby's PYBStick26, Machdyne,
  nullbits, Pimoroni, Seeed Studio, SparkFun, 0xCB), every electrical fact cited to that board's own upstream firmware source and
