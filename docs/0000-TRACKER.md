...
- [ ] [0049] document external devices/boards + how a user writes their own | **Proposed, docs-design + one narrow prerequisite landed, main scenarios still no code.** Scope widened 2026-08-16 from "devices" to "devices and boards": a custom device on an existing board is already fully solved (`demo/eink_run.py`, zero new code); a custom `BoardSpec` or a fully custom firmware+flash-layout board are design-only so far (`BaseDevice`/`MicroPythonDevice`/`KalumaDevice`/`load_flash.py` all need real new parameters, not yet written) — see the record's "Design update" section for the 3-scenario breakdown and a `CustomBoardSetup`/`CustomFlashLayout` API sketch. Its one flagged prerequisite **was implemented same-day, and grew into a small schema reshape**: `load_flash.py`'s three hardcoded `*_FS_BLOCKSIZE` constants are gone, `fs_blocksize` is now a real per-board `layout` key — and while moving it, `firmware_specs.json`'s `flash_layout`/`default_tag` (previously sibling top-level dicts keyed by the same board-name string as `boards` purely by convention) got nested into a new `BoardFirmwareSpec` per board instead, so everything board-specific lives in one place (`FirmwareSpec.boards: dict[str, BoardFirmwareSpec]`). Also fixed: `scripts/fetch_firmware.py` (regeneration preserves the new shape, never overwrites an existing board's `default_tag`) and every caller/test that touched the old flat shape (`cli/mklittlefs.py`, `tests/test_firmware_retrieve.py`, `tests/test_cli.py`, `tests/test_cli_mklittlefs.py`). No behavior change for `pico`/`pico_w` (`fs_blocksize` still `4096`, `default_tag`s unchanged) — verified by the full `uv run pre-commit run --all-files` pass (mypy/ruff/pytest × pure-Python and native); 0035 revisited inline (it had asserted block size "never varies" when `fs_start`/`fs_blockcount` first went per-board, and described the now-removed single `FirmwareSpec.flash_layout` field). `README.md` now has a short pointer section ("External devices & custom boards"); `whether the CLI should grow a --device/--board-spec hook` was considered in depth and resolved **no, for now** (API-only, matching 0029). Still open: whether `ExternalDevice`'s attach-only surface is public-API-ready, and whether `demo/eink_run.py` gets promoted into a dedicated `reference/` how-to
