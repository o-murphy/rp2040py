...
- [ ] [0045](records/0045-cyw43-nat-libslirp-cython.md) CYW43 step 4 NAT bridge: embed `gVisor`'s `pkg/tcpip` via `cgo` | Proposed, revised twice same day (2026-08-14): `PyTCP` embedding tried and resolved **negative** (real `pytcp==2.7.6` PoC — unconditional TX src-ownership check, no bypass); `gVisor` fallback empirically **confirmed working** (real Go program via `SagerNet/gvisor` fork — `SetPromiscuousMode`+`SetSpoofing` close both gaps, verified by an actually-transmitted spoofed-source SYN-ACK); `lwIP` and 4 Rust candidates surveyed too, `gVisor` is the only one with empirical proof; user then picked `cgo` shared-library integration (iOS caveat checked, moot — project's own `cibuildwheel` matrix has no iOS target) over a Go subprocess — Python-side binding mechanism, fork-vendoring-vs-pinning, Go-toolchain-in-CI, and the 4a/4b sub-step breakdown all still open, no code written yet
