...
- [ ] [0049] document external devices/boards + how a user writes their own | **Proposed. Docs-only pieces landed; the board-authoring design is accepted, phases 1-4 of its 5-phase implementation plan landed 2026-08-16/17, only phase 5 (tests/how-to) remains.** Scope widened 2026-08-16 from "devices" to "devices and boards": a custom device on an existing board is already fully solved (`demo/eink_run.py`, zero new code). **Accepted design** (record's "Accepted design" section): `BoardSpec` extended in place (kept its name - an earlier draft renamed it to `BoardSetup`, reverted before any code existed, see the record's own "Naming" note) with `layout` + a resolved local-path-only `image` - now (phase 3) the *only* thing `BaseDevice`/`MicroPythonDevice`/`KalumaDevice` accept for "which board", a keyword-only `board: BoardSpec` param with no separate `image` arg and no board-name string internally; `resolve_board_spec(board, firmware_spec, tag)` is the one shared shortcut both `--board` (CLI) and SDK callers use for a known board; CLI (phase 4) got `--board-spec target:attr` + `RP2040PY_BOARD_SPEC` env var (file-or-module + attr, resolving a hand-built `BoardSpec`), mutually exclusive with `--board` and (on `micropython`/`kaluma`) `--image`/`--fetch-fw-only` or (on `mklittlefs`) `--target`, scoped to `micropython`/`kaluma`/`mklittlefs`/`run` (not `bench` - that still resolves `board: str` unchanged, purely because nobody's asked for it yet; see the record's "CLI scope, resolved" note; `mklittlefs` sizes from `board.layout` directly, still overridable by explicit `--block-size`/`--block-count`), `--circuitpython` stays compatible since it also picks the loader/dump function and post-boot console behavior, not just firmware-family resolution. Breaking `MicroPythonDevice`/`KalumaDevice`'s public constructor signature (phase 3) shipped as accepted, not deprecation-shimmed - no doc had ever promised `board`/`image`'s old shape as stable SDK API. `_run_async`'s own "not settled" sub-question is now resolved: `run`'s `--image` is a raw program with no `FirmwareSpec` to resolve against, so it stays fully independent of `--board`/`--board-spec` (a new `_resolve_run_mcu()` handles just the mcu/extras choice, the only real conflict being `--board`+`--board-spec` together) - `bench` alone keeps `build_rp2040(str)` (now a thin wrapper around new `build_rp2040_from_spec()`) unchanged. **Implemented 2026-08-16**: the `fs_blocksize` prerequisite (`load_flash.py`'s three hardcoded `*_FS_BLOCKSIZE` constants replaced by a real per-board `fs_blocksize` `layout` key, `firmware_specs.json`'s `flash_layout`/`default_tag` reshaped into `FirmwareSpec.boards: dict[str, BoardFirmwareSpec]`); then, on a separate explicit go-ahead, **phases 1-2** (`boards.py` gained `FlashLayout`/`BoardSpec.layout`/`BoardSpec.image`/`resolve_board_spec()`, zero behavior change; `load_flash.py`'s six load/dump functions took a resolved `FlashLayout` instead of re-deriving one from a board-name string, via temporary `_layout()` bridge helpers in `mp_device.py`/`kaluma_device.py`); then **2026-08-17, phases 3-4** (the breaking constructor change + CLI `--board-spec` wiring described above), **live-verified against real MicroPython `1.28.0` firmware** both by hand (mutual-exclusion, incompatible-flag, and bad-`attr` error paths all exercised directly) and via a new CI job (`tests/pico_spec.py` + `ci-micropython.yml`'s `test-board-spec` job, both `--board-spec` and `RP2040PY_BOARD_SPEC` forms). Every step verified by the full `uv run pre-commit run --all-files` pass; 0035 revisited inline (its "block size never varies" claim, and its description of the now-removed single `FirmwareSpec.flash_layout` field). `README.md` has a short pointer section ("External devices & custom boards") to this record. Still open: phase 5 (updating `tests/test_boards.py`/`test_firmware_retrieve.py`/`test_cli_mklittlefs.py` for the reshape, writing the actual how-to doc - `tests/pico_spec.py` is a worked example it can point to); whether `ExternalDevice`'s attach-only surface is public-API-ready; whether `demo/eink_run.py` gets promoted into a dedicated `reference/` how-to
