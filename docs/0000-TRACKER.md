...
- [ ] [0057] RESET button / RUN pin reset hook on `RP2040` | documented, not built - the full hard reset exists once, privately, on `BaseDevice` rather than `RP2040`, which is the placement half of this record's own blocker; [0087] only needs it as a fallback
