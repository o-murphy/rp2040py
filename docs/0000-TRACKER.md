...
- [ ] [0066] board support expansion: which RP2040 boards are addable, and what each still needs | **Partly worked through.** Its "addable now, has a MicroPython port" checklist is **fully built** as of [0080] (12/12, records [0068]-[0071] and [0073]-[0080]); what remains open is the CircuitPython-only addable list (37 boards, **4 done** - [0081]/[0082]/[0083]/[0084]) and everything gated behind a missing `ExternalDevice`. 128 boards checklisted across both MicroPython's and CircuitPython's board lists - 49 addable with zero new devices, 59 need one or more device (grouped by missing chip where shared across boards); 17 RP2350 boards and 3 architecturally-unmodelable ones flagged separately, not dropped
- [ ] [0088] USB host side: mass storage, CDC control lines, and reset | **Last in the agreed order** ([0087]'s Sequencing). **Documented, measured, nothing built.** `usb/cdc.py` is a CDC-ACM consumer, not a USB host - it claims one interface and ignores the rest. DTR/RTS *do* work (firmware reports `serial_connected: True`, and TinyUSB's `tud_cdc_connected()` is the DTR bit) but cannot be changed after enumeration; `SET_LINE_CODING` is absent, and a 1200-bps-touch reset would still do nothing because rp2 defines neither `MICROPY_HW_USB_CDC_1200BPS_TOUCH` nor `..._DTR_RTS_BOOTLOADER` and CircuitPython has no such handler - [0057]'s reset hook is the cheap capability instead. Mass storage only buys fidelity over `--dump-fs`, and is mutually exclusive with [0087]
- [ ] [0087] CircuitPython's CIRCUITPY is writable over the raw REPL we already have | **Documented, measured, nothing built.** No separate CircuitPython raw REPL exists to implement - `raw_repl.py` is family-agnostic and already used for both - and `storage.remount('/', readonly=False)` *succeeds* here, because CircuitPython gates writes on a blockdev lock its MSC callbacks take only on real SCSI traffic and `usb/cdc.py` claims nothing but the CDC interface. So the firmware can write its own filesystem (LFN and subdirectories included), real `mpremote exec`/`run` work over `--tcp-port`, and `mpremote fs *` does not (CircuitPython has no `os.ilistdir`). Corrects [0085]; changes what [0086] is deciding
- [ ] [0086] a FAT12 library dependency + a `mkfat12` CLI subcommand | **Second in the agreed order** ([0087]'s own Sequencing section). **Proposed, plan and library survey only.** The demo half shipped in [0085]; this is what needs an API decision first - an optional extra shaped like `fs = ["littlefs-python"]`, and a `mklittlefs`-shaped subcommand. Survey: `pyfatfs` works and is MIT but drags PyFilesystem2/`pkg_resources` and needs a `setuptools<81` pin, `PyFAT12` is mispackaged on PyPI (installs a `path` module, not `pyfat12`), `FATtools` is the most capable and is GPLv3 against this project's MIT; growing the existing 8.3 builder's own LFN support stays the no-dependency fourth option
