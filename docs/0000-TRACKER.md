...
- [x] [0027](records/0027-cyw43-wifi.md) CYW43439 / Pico W WiFi (epic) | steps 0-3g done, live-boot-verified on both v1.23.0 and v1.28.0 (2026-08-13/14, see 0042/0043); step 4 (real network bridge) implemented + live-boot-verified via [0048](records/0048-cyw43-nat-reflector.md) (2026-08-16) — `main-spi.py` hang fixed but confirmed unrelated to this epic, see 0044. Only UDP/DNS relay (hostname-based connections) remains deferred, not scheduled
- [x] [0048](records/0048-cyw43-nat-reflector.md) CYW43 step 4 NAT bridge: custom minimal hand-rolled TCP reflector (supersedes 0045's engine choice) | Implemented — verified (2026-08-16): guest-facing leg reuses `bus.py`'s own lossless in-order FIFO, so no retransmission/congestion-control/reassembly needed; host-facing leg is a real `asyncio` socket; 4a (MAC fix)/4b (DHCP)/4c (ARP)/4d (TCP splice) all landed, unit/integration-tested, and live-boot-verified on both v1.23.0/v1.28.0 (`main-cyw43.py` completes a real TCP round trip to `1.1.1.1:80` through the reflector)
