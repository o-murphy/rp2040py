...
- [x] [0048](records/0048-cyw43-nat-reflector.md) CYW43 step 4 NAT bridge: custom minimal hand-rolled TCP reflector + DNS relay (supersedes 0045's engine choice) | Implemented — verified (2026-08-16): guest-facing leg reuses `bus.py`'s own lossless in-order FIFO, so no retransmission/congestion-control/reassembly needed; host-facing leg is a real `asyncio` socket; 4a (MAC fix)/4b (DHCP)/4c (ARP)/4d (TCP splice)/4e (one-shot UDP DNS relay) all landed, unit/integration-tested, and live-boot-verified on both v1.23.0/v1.28.0 — `main-cyw43.py` completes a real TCP round trip to `1.1.1.1:80` through the reflector, and `mip.install()` (which failed with `OSError -2` before 4e) now resolves `micropython.org` and installs a real package. **Not the full picture** - real remote RST wrongly surfaces as a clean FIN, `disconnect()` is a no-op, AP mode/non-DNS UDP/IPv6/multi-guest are all unbuilt (full inventory in the record's own "Known gaps" section, 2026-08-16); a `windows-latest` CI flake was also found + fixed same day (fixed-`asyncio.sleep()` race in the test suite) but not yet re-pushed to confirm
