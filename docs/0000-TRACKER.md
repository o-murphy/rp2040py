...
- [ ] (no record yet) **CircuitPython 10.x never reaches the REPL** (9.2.9/8.0.2 do) | not root-caused - see [docs/tasks/circuitpython-10x-boot-stall.md](tasks/circuitpython-10x-boot-stall.md); zero console output indefinitely, reproduces on plain `--board pico` so **not** a CYW43 issue. Boot goes quiet right after touching three unimplemented blocks (XIP_CTRL, ROSC `RANDOMBIT`, PIO `clkDivRestart`) then spins in an unlogged loop; the missing inter-core FIFO was checked and ruled out. Found 2026-08-16 while closing 0048's CircuitPython gap. `README.md` currently advertises `--circuitpython` with `--image 10.2.1` as its example, which is exactly the broken combination
