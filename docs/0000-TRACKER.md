...
- [ ] [0045](records/0045-cyw43-nat-libslirp-cython.md) CYW43 step 4 NAT bridge: embed `PyTCP`, optional runtime dependency | Proposed — supersedes this record's own earlier same-day `libslirp`/Cython text (glib/pkg-config had no cross-platform story); `bus.py`/`chip.py` stay pure Python unchanged; implementation not started, load-bearing PyTCP-embedding question unverified
