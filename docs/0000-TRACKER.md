...
- [ ] [0089] one reset for every trigger: soft vs hard, guest- vs host-initiated | **Phases 0-2 landed; Phase 3 dropped as unwanted; Phases 4-6 not started** (2026-08-20) - Phase 3 (a host-side soft-reset API) was built, live-verified and then removed on the maintainer's call: a soft reset is bytes the firmware already answers (console Ctrl-D, `mpremote soft-reset`, or one line of `aexec` - `machine.soft_reset()`/`supervisor.reload()`, measured to return cleanly and leave the console usable on both families), and what matters is that guest-initiated `machine.reset()` behaves like hardware - which is Phases 1/2/5. Its findings (D2, the transcripts) survive as documentation. - Phase 2 shipped the host-initiated hard reset: `BaseDevice.hard_reset_async()`/`ahard_reset()` under `MicroPythonDevice`'s REPL lock, sharing `_aconnect()`'s arm/wait halves; live-verified on MP v1.23.0 and CP 10.2.1 (`tests/hard_reset_run.py`, now CI on both workflows), where CircuitPython reports `RESET_PIN` - the one cell the record could previously only derive. Known gap, pre-existing: CP **8.0.2** never re-enumerates after a chip reset (spins in SRAM) - Phase 5's problem, CI gates that step. Phase 1 shipped reset-cause fidelity: `ResetCause` + `BaseDevice.hard_reset(cause=...)`, `RPWatchdog.reset()`, `RPVREGAndChipReset.record_reset_cause()`, and `RP2040.vreg_and_chip_reset` named in both twins; a watchdog reboot keeps its own REASON/SCRATCH while a RUN-pin/power-on reset clears them and records `HAD_RUN`/`HAD_POR` - live-boot CI on both families (`main-reset-cause.py`). Phase 0 shipped the two prerequisites: the post-boot handshake now lives on `MicroPythonDevice._post_boot_handshake()` (a bare newline for both families - measured: the CLI's old CircuitPython Ctrl-C kills a running `code.py` and buys nothing on an idle REPL - called from `_aconnect()`, out of the CLI - closes [0087]'s item 4, live-boot verified on MP v1.23.0 + CP 8.0.2), and the hard-reset sequence is a real `BaseDevice.hard_reset()` with `_on_watchdog_trigger()` reduced to a caller of it. Design half: now the umbrella record for reset across [0057]/[0066]/[0087]/[0088]: one hard-reset owner on `BaseDevice` + one soft-reset entry point on `MicroPythonDevice`, every trigger a caller, with a reset-cause table (`machine.reset_cause()`/`microcontroller.cpu.reset_reason`) as the fidelity bar. All open questions decided; measured against real MP v1.23.0/CP 10.2.1 - a bare Ctrl-D at the **raw** prompt does *not* re-run `main.py`/`code.py` in either family (only one at the friendly prompt does), and a REPL-written `code.py` does not auto-reload. Mass storage (MSC/UF2 bootloader) explicitly out of scope
...
- [ ] [0057] RESET button / RUN pin reset hook on `RP2040` | documented, not built, but its register half is: [0089]'s Phase 1 landed the `REASON=0`/`CHIP_RESET=HAD_RUN` answer in code, and its Phase 4 is where the pin itself lands - the full hard reset exists once, privately, on `BaseDevice` rather than `RP2040`, which is the placement half of this record's own blocker; [0087] only needs it as a fallback
...
- [ ] [0065] `test_a_queued_exec_erroring_does_not_stall_the_ones_behind_it` flaky on CI | **Closed dormant, not root-caused** (2026-08-18; one local sighting under load 2026-08-20 - so not CI- or Windows-specific, and there is now a repro recipe) - single 2026-08-14 occurrence, not reproduced in ~85 subsequent CI runs; reopen if it recurs

### Implemented

- [x] [0090] the post-boot nudge is a newline, for both firmware families | measured, then changed: Ctrl-C (what the CLI sent CircuitPython) is byte-identical to a newline on an *idle* REPL and kills a running `code.py`/`main.py` otherwise - it would have broken three `--littlefs ... --expect-text` CI jobs and `demo/lcd_run.py --code`. Fell out of [0089]'s Phase 0.1 moving the nudge onto the device
...
- [x] [0036] `--littlefs`/`--fat12` mutual exclusivity | explicit validation instead of a silent drop; **follow-through 2026-08-20** finished what it deferred - the format follows the firmware family, so `MicroPythonDevice` takes one `filesystem=` argument and `_validate_filesystem()` both validates and resolves it, now also rejecting the *mismatched* flag (`--fat12` without `--circuitpython`, and vice versa)
