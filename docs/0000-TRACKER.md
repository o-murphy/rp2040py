...
- [ ] [0045](records/0045-cyw43-nat-libslirp-cython.md) CYW43 step 4 NAT bridge: embed `PyTCP`, optional runtime dependency | Proposed, load-bearing question resolved **negative** (2026-08-14) — verified against real `pytcp==2.7.6` that PyTCP cannot originate traffic for a destination it doesn't own (independent RX dst-ownership and TX src-ownership gates, the latter unconditional with no bypass); `lwIP`/`gVisor` fallbacks evaluated same session — `gVisor` closes both gaps via documented `SetPromiscuousMode`/`SetSpoofing` API (same architecture Tailscale ships in production), `lwIP` needs NAPT ported from a downstream fork; no pick made yet, both need a real Cython/`cgo` binding designed - not started
