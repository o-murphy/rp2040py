...
- [ ] [0049] document external devices/boards + how a user writes their own | **Proposed. Docs-only pieces landed; the board-authoring design is accepted but its 5-phase implementation plan hasn't started.** Scope widened 2026-08-16 from "devices" to "devices and boards": a custom device on an existing board is already fully solved (`demo/eink_run.py`, zero new code). **Accepted design** (record's "Accepted design" section): `BoardSpec` renamed/extended into `BoardSetup` (mcu + extras + `layout` + a resolved local-path-only `image`) - the *only* thing `BaseDevice`/`MicroPythonDevice`/`KalumaDevice` ever accept for "which board", never a board-name string internally; `resolve_board_setup(board, firmware_spec, tag)` is the one shared shortcut both `--board` (CLI) and SDK callers use for a known board; CLI gets `--board-spec target:attr` (file-or-module + attr, resolving a hand-built `BoardSetup`), mutually exclusive with `--board`/`--image`/`--target`(mklittlefs)/`--fetch-fw-only` (worked out flag-by-flag, `--circuitpython` stays compatible - it also picks the loader/dump function and post-boot console behavior, not just firmware-family resolution), plus an `RP2040PY_BOARD_SPEC` env var (deliberately *not* `conftest.py`-style auto-discovery). Breaking `MicroPythonDevice`/`KalumaDevice`'s public constructor signature is accepted as fine, not deprecation-shimmed - no doc has ever promised `board`/`image`'s current shape as stable SDK API. **Implemented same-day, narrower in scope than the design above**: the one flagged prerequisite, `load_flash.py`'s three hardcoded `*_FS_BLOCKSIZE` constants replaced by a real per-board `fs_blocksize` `layout` key - grew into reshaping `firmware_specs.json`'s `flash_layout`/`default_tag` (previously sibling top-level dicts keyed by the same board-name string as `boards` purely by convention) into a proper nested `BoardFirmwareSpec` per board (`FirmwareSpec.boards: dict[str, BoardFirmwareSpec]`), with `scripts/fetch_firmware.py` and every touched caller/test updated to match. No behavior change for `pico`/`pico_w` - verified by the full `uv run pre-commit run --all-files` pass; 0035 revisited inline (its "block size never varies" claim, and its description of the now-removed single `FirmwareSpec.flash_layout` field). `README.md` has a short pointer section ("External devices & custom boards") to this record. Still open: whether `ExternalDevice`'s attach-only surface is public-API-ready, whether `demo/eink_run.py` gets promoted into a dedicated `reference/` how-to, and whether `run`'s own `--image` folds into `BoardSetup.image` or stays separate (raw program, not board-family-versioned firmware)
