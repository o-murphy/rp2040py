...
- [ ] [0059] firmware resolution inside `BoardSpec`: one path for `--board` and `--board-spec` | **Proposed, nothing implemented - agreed to be the first thing built next session.** `BoardSpec` gains one declarative field, `firmware: dict[family, BoardFirmwareSpec]` (an *existing* type, keyed by the family names `firmware_specs.json` already uses), and resolution moves from import time to use time. Unifies two levels at once: the board spec *and* the firmware spec, between built-in and custom boards - the only remaining difference being who maintains the data (`scripts/fetch_firmware.py` regenerates it for shipped boards; a custom board's author edits it by hand, since no API enumerates releases for a third-party board). Subsumes, rather than separately fixes, two live defects: a board file downloading firmware as a side effect of `import` (which made offline `--board-spec` need cache-seeding during [0056]), and `--image` being incompatible with `--board-spec` - so one row of [0049]'s flag table is superseded. Also lets one file serve several firmware families (which is why [0056]'s board exists twice today), and drops `image` from the authoring surface entirely: a declaration may be a URL *or* a local path, so the smallest hand-written board is one `firmware` entry and nothing else, and it is the precondition that makes shipping community boards in the package answerable at all - the record carries the promotion checklist for `boards.BOARDS` (upstream-derived numbers, a `firmware_specs.json` entry, CI live-boot, devices already in `rp2040py.external`, a named maintainer) and rejects a third `community` tier
